...
# Admin features removed for MVP
...
def delete_user(user_id: int, db: Session = Depends(get_db)):
    ...
    # Admin protections removed in MVP; this endpoint is not exposed in UI
...
def create_admin_user(user_data: dict, db: Session = Depends(get_db)):
...
def admin_repair_user_stats(db: Session = Depends(get_db)):
    users = db.query(User).all()
    repaired = 0
    for user in users:
        quiz_progress_list = db.query(UserQuizProgress).filter(UserQuizProgress.user_id == user.id).all()
...
def admin_reset_user_progress(user_id: int, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def admin_cleanup_null_activity(db: Session = Depends(get_db)):
