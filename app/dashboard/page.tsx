              ...
              <CardContent>
                <div className="grid md:grid-cols-2 gap-6">
                  {subjects.map((subject) => (
                    <div
                      key={subject.id}
                      className="border border-slate-200 rounded-lg p-4 hover:shadow-md transition-shadow flex flex-col h-full"
                    >
                      ...
                      <div className="space-y-2 mb-4">
                        {subject.levels.map((level: any) => (
                          <div className="flex items-center justify-between" key={level.id}>
                            <div
                      ...
                      {/* Start/Continue/Review Button (stick to bottom) */}
                      {(() => {
                        const firstUnlocked = subject.levels.find(
                          (level: any) => level.unlocked && !level.completed
                        );
                        return (
                          <Link href={`/subject/${subject.id}`} className="mt-auto block">
...
<div className="space-y-4">
  {recentActivity.length > 0 ? (
    ...
    <div className="text-center text-slate-600 py-8">
      ...
      <p className="mt-2">Start learning and your recent activity will appear here!</p>
    </div>
  )}
</div>
              </CardContent>
